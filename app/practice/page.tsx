      ...
      <div className="w-full mt-2 animate-in fade-in slide-in-from-bottom-4 zoom-in-95 duration-700" style={{ animationFillMode: "both" }}>
        <Tabs defaultValue="maths" value={activeTab} onValueChange={setActiveTab} className="w-full flex flex-col gap-8">
          <div className="flex w-full justify-start overflow-x-auto pb-2 scrollbar-none">
            <TabsList className="h-13 p-1.5 bg-card/95 md:bg-card/60 backdrop-blur-none md:backdrop-blur-xl border-2 border-border/40 rounded-2xl shadow-xs gap-1">
              {TAB_DATA.map((tab) => (
                <TabsTrigger 
                  key={tab.id} 
                  value={tab.id}
              ...
              {TAB_DATA.map((tab) => 
                activeTab === tab.id && (
                  <TabsContent key={tab.id} value={tab.id} className="mt-0 outline-none" asChild forceMount>
                    ...
                    >
                      {/* Subject Banner */}
                      <div className={`w-full rounded-3xl border-2 bg-card/95 md:bg-card/60 backdrop-blur-none md:backdrop-blur-2xl ${tab.bgClass} p-5 sm:p-6 sm:px-7 flex flex-col sm:flex-row items-start sm:items-center gap-4 shadow-xl shadow-black/5 overflow-hidden relative`}>
