...
1. **Freestyle Question Mode**: No timer, question limits (10, 20, 30), and correct lobby configurations.
...
- **Lobby View**:
  - Update `handleModeChange`:
    - Timed: `engine.setConfig("timed", difficulty, 30, null)`
    - Freestyle: `engine.setConfig("freestyle", difficulty, null, 10)`
  - Update the selector so that if `mode === "freestyle"`, it reads and updates `engine.state.questionLimit` using a new `handleQuestionLimitChange` helper.
- **Active Game View**:
