...
2. **Dynamic Progress Bar (Completed)**: Progress bar displaying status and progress based on active mode.
3. **Attempt History Tracking**: Logging each question, user response, correct answer, and correct/wrong/skipped status, with a scrollable list view on the scorecard.
...
- **Active Game View (Completed)**:
  - Dynamic Progress Bar is already implemented.
- **Scorecard View**:
